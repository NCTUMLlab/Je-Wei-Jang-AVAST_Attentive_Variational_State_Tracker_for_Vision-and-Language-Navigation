...
def run_data_parallel(
    ...
    while True:
        if evaluate:
            ...
            actions, outputs, candidate_action_features, hiddens, dists = agent.act_with_dists(
                location_infos=location_infos,
                instr_embed=instr_embed,
                instr_mask=instr_mask,
                last_action_features=last_action_features,
        ...
        if not evaluate:
            for parallel_idx, done in enumerate(dones):
                if done <= 1:
                    ...
                    pair_datas['posterior_means'].append(
                        dists['enc_mean'][parallel_idx: parallel_idx + 1]
                    )
                    pair_datas['posterior_stds'].append(
                        dists['enc_std'][parallel_idx: parallel_idx + 1]
                    )
                    pair_datas['prior_means'].append(
                        dists['prior_mean'][parallel_idx: parallel_idx + 1]
                    )
                    pair_datas['prior_stds'].append(
                        dists['prior_std'][parallel_idx: parallel_idx + 1]
                    )
